...
router.get('/', middleware.checkAuthenticated, function(req, res) {
	var sqlReq = new sql.Request().query("SELECT * FROM tbl_sponsors ORDER BY sponsor_date ASC").then(result => {
...
router.get('/edit/:id', middleware.checkAuthenticated, function(req, res) {
	var sqlReq = new sql.Request().input("id", sql.Int, req.params.id).query("SELECT TOP 1 * FROM tbl_sponsors WHERE id = @id")
		.then(result => {
...
router.put('/edit/:id', middleware.checkAuthenticated, function(req, res) {
	...
	var sqlQuery = "UPDATE tbl_sponsors " +
...
router.post('/new', middleware.checkAuthenticated, function(req, res) {
	...
	var sqlQuery = "INSERT INTO tbl_sponsors (name, deleted, sponsor_date, image_url, refer_url) VALUES " +
