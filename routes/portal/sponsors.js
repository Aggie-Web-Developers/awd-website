...
router.get('/edit/:id', middleware.checkAuthenticated, function(req, res) {
	var sqlReq = new sql.Request().input("id", sql.Int, req.params.id).query("SELECT TOP 1 * FROM tbl_sponsors WHERE id = @id", (err, result) => {
		if (err){
			req.flash("error", "Error loading sponsors.");
...
router.put('/edit/:id', middleware.checkAuthenticated, function(req, res) {
	...
	var queryText = "UPDATE tbl_sponsors " +
...
router.post('/new', middleware.checkAuthenticated, function(req, res) {
	...
	var queryText = "INSERT INTO tbl_sponsors (name, deleted, sponsor_date, image_url, refer_url) VALUES " +
